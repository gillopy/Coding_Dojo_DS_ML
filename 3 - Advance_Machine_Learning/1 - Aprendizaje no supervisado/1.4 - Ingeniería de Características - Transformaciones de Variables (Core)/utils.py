...
def plot_distributions2(data, 
                       ...
                       kde=False,
                       kde_kws=None,
                       show_iqr_and_legend=True):  # Parámetro adicional para manejar IQR y leyenda
    """
    Genera histogramas para todas las columnas numéricas de un dataset, incluyendo métricas IQR y KDE opcional.

    Parámetros:
    -----------
    show_iqr_and_legend : bool, opcional
        Si es True, muestra tanto los rangos IQR (Q1, Q2, Q3, IQR) como la leyenda asociada.
        Si es False, oculta ambos.
    """
    import matplotlib.pyplot as plt
    import seaborn as sns
    import numpy as np
    ...
    for i, col in enumerate(numeric_cols):
        if i < len(axes):
            ...
            sns.histplot(
                ...
                linewidth=linewidth,
                ax=ax,
                kde_kws=kde_kws if kde else {}  # Pass KDE params if kde is True
            )
            ax.set_xlabel("")  # Esta línea elimina el título del eje x
            
            if show_iqr_and_legend:  # Mostrar rangos IQR y leyenda solo si es True
                # Cálculo de métricas
            ...
            # Configuración de bordes y ticks
            for spine in ['top', 'right', 'left', 'bottom']:
            ...
            # Mostrar leyenda solo si show_iqr_and_legend es True
...
def plot_top_correlations(data, 
                          ...
                          palette="Set2", 
                          figsize=(12, 8), 
                          title="Top Features Correlacionados", 
                          xlabel="Correlation coefficient", 
                          ylabel="Features", 
                          grid=True, 
                          grid_style=None):
    """
    Genera una gráfica de barras horizontales mostrando las variables con mayor correlación
    absoluta con una variable objetivo en un dataset.
    ...
    target_column : str
        Nombre de la columna objetivo con la cual calcular las correlaciones.
    top_n : int, opcional
        Número de características con las correlaciones más altas a graficar (por defecto 10).
    palette : str, opcional
        Paleta de colores de Seaborn (por defecto "Set2").
    figsize : tuple, opcional
        Tamaño de la figura (ancho, alto) (por defecto (12, 8)).
    title : str, opcional
        Título de la gráfica (por defecto "Top Features Correlacionados").
    xlabel : str, opcional
        Etiqueta del eje X (por defecto "Correlation coefficient").
    ylabel : str, opcional
        Etiqueta del eje Y (por defecto "Features").
    grid : bool, opcional
        Si es True, muestra una cuadrícula (por defecto True).
    grid_style : dict, opcional
        Estilo personalizado para la cuadrícula (por defecto None).
    ...
    colors = sns.color_palette(palette, len(top_features))
    ...
    # Configuración opcional de la cuadrícula
    if grid:
        default_grid_style = {'linestyle': '--', 'linewidth': 0.6, 'color': 'gray', 'alpha': 0.5}
        grid_style = grid_style or default_grid_style
